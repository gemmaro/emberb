...
`false`, and `true` are prefixed with `Q` e.g. `RUBY_Qnil`.[^undef]
As a convenience, `RUBY_Qfalse` is also false in C (`0`).
...
`RUBY_Qnil` if no exception occurred). Unlike in Ruby, you must manually clear
...
(rescue) function. By having these return, say, `RUBY_Qtrue` and `RUBY_Qfalse`
you can detect which case you are in.
...
it returns `RUBY_Qnil` and sets `state` to some nonzero value. If you want to
...
You undefine a constant by setting it to `RUBY_Qundef`. Getting a constant's
`VALUE` is a little nuanced. The API function you call depends on what
you want to happen if the constant is _not_ defined in the module you specify:
...
[^undef]: There's also `RUBY_Qundef` representing an undefined value, but
          this has no Ruby equivalent and is [rarely](#handling-blocks)
          [used](#constants-1). In fact, outside of those rare occasions,
          `RUBY_Qundef` can segfault the VM if Ruby was expecting a normal `VALUE`.
